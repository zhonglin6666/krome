...
// FilterActivePods return pods that have not terminated.
func FilterActivePods(pods []*v1.Pod) []*v1.Pod {
	...
	for _, p := range pods {
		if IsPodActive(p) {
			...
			logrus.Infof("Ignoring inactive pod %v/%v in state %v, deletion time %v",
...
// IsPodActive return pod that have not terminated.
