...
func (r *ReconcileDeployment) rolloutRolling(d *kromev1.Deployment, rsList []*kromev1.ReplicaSet) error {
	...
	scaledDown, err := r.reconcileOldReplicaSets(allRSs, filterActiveReplicaSets(oldRSs), newRS, d)
	if err != nil {
		return err
	}
	if scaledDown {
...
func (r *ReconcileDeployment) reconcileOldReplicaSets(allRSs []*kromev1.ReplicaSet, oldRSs []*kromev1.ReplicaSet, newRS *kromev1.ReplicaSet, deployment *kromev1.Deployment) (bool, error) {
	...
	logrus.Infof("Cleaned up unhealthy replicas from old RSes by %d", cleanupCount)
	...
	logrus.Infof("Scaled down old RSes of deployment %s by %d", deployment.Name, scaledDownCount)
...
func (r *ReconcileDeployment) cleanupUnhealthyReplicas(oldRSs []*kromev1.ReplicaSet, deployment *kromev1.Deployment, maxCleanupCount int32) ([]*kromev1.ReplicaSet, int32, error) {
	sort.Sort(ReplicaSetsByCreationTimestamp(oldRSs))
	...
	for i, targetRS := range oldRSs {
		...
		logrus.Infof("Found %d available pods in old RS %s/%s", targetRS.Status.AvailableReplicas, targetRS.Namespace, targetRS.Name)
...
func (r *ReconcileDeployment) scaleDownOldReplicaSetsForRollingUpdate(allRSs []*kromev1.ReplicaSet, oldRSs []*kromev1.ReplicaSet, deployment *kromev1.Deployment) (int32, error) {
	maxUnavailable := maxUnavailable(*deployment)
	...
	logrus.Infof("Found %d available pods in deployment %s, scaling down old RSes", availablePodCount, deployment.Name)
