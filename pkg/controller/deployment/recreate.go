...
func (r *ReconcileDeployment) rolloutRecreate(d *kromev1.Deployment, rsList []*kromev1.ReplicaSet, podMap map[types.UID]*v1.PodList) error {
	...
	activeOldRSs := filterActiveReplicaSets(oldRSs)
...
func (r *ReconcileDeployment) scaleDownOldReplicaSetsForRecreate(oldRSs []*kromev1.ReplicaSet, deployment *kromev1.Deployment) (bool, error) {
	scaled := false
	for i := range oldRSs {
		rs := oldRSs[i]
...
func oldPodsRunning(newRS *kromev1.ReplicaSet, oldRSs []*kromev1.ReplicaSet, podMap map[types.UID]*v1.PodList) bool {
	if oldPods := getActualReplicaCountForReplicaSets(oldRSs); oldPods > 0 {
		return true
	}
...
func (r *ReconcileDeployment) scaleUpNewReplicaSetForRecreate(newRS *kromev1.ReplicaSet, deployment *kromev1.Deployment) (bool, error) {
