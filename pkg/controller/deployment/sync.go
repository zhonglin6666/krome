...
func (r *ReconcileDeployment) syncStatusOnly(d *kromev1.Deployment, rsList []*kromev1.ReplicaSet) error {
...
// are paused.
func (r *ReconcileDeployment) sync(d *kromev1.Deployment, rsList []*kromev1.ReplicaSet) error {
	newRS, oldRSs, err := r.getAllReplicaSetsAndSyncRevision(d, rsList, false)
	if err != nil {
		return err
	}
...
func (r *ReconcileDeployment) checkPausedConditions(d *kromev1.Deployment) error {
	if !hasProgressDeadline(d) {
		return nil
	}
	cond := getDeploymentCondition(d.Status, kromev1.DeploymentProgressing)
	...
	if d.Spec.Paused && !pausedCondExists {
		condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionUnknown, deploymentutil.PausedDeployReason, "Deployment is paused")
		setDeploymentCondition(&d.Status, *condition)
		needsUpdate = true
	} else if !d.Spec.Paused && pausedCondExists {
		condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionUnknown, deploymentutil.ResumedDeployReason, "Deployment is resumed")
		setDeploymentCondition(&d.Status, *condition)
		needsUpdate = true
	}
	...
	var err error
	d, err = r.kromeClient.AppsV1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d, metav1.UpdateOptions{})
	return err
}
...
func (r *ReconcileDeployment) getNewReplicaSet(d *kromev1.Deployment, rsList, oldRSs []*kromev1.ReplicaSet, createIfNotExisted bool) (*kromev1.ReplicaSet, error) {
	...
	if existingNewRS != nil {
		...
		annotationsUpdated := setNewReplicaSetAnnotations(d, rsCopy, newRevision, true, maxRevHistoryLengthInChars)
		minReadySecondsNeedsUpdate := rsCopy.Spec.MinReadySeconds != d.Spec.MinReadySeconds
		if annotationsUpdated || minReadySecondsNeedsUpdate {
			rsCopy.Spec.MinReadySeconds = d.Spec.MinReadySeconds
			return r.kromeClient.AppsV1().ReplicaSets(rsCopy.ObjectMeta.Namespace).Update(context.TODO(), rsCopy, metav1.UpdateOptions{})
		}

		// Should use the revision in existingNewRS's annotation, since it set by before
		needsUpdate := setDeploymentRevision(d, rsCopy.Annotations[deploymentutil.RevisionAnnotation])
		// If no other Progressing condition has been recorded and we need to estimate the progress
		// of this deployment then it is likely that old users started caring about progress. In that
		// case we need to take into account the first time we noticed their new replica set.
		cond := getDeploymentCondition(d.Status, kromev1.DeploymentProgressing)
		if hasProgressDeadline(d) && cond == nil {
			msg := fmt.Sprintf("Found new replica set %q", rsCopy.Name)
			condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionTrue, deploymentutil.FoundNewRSReason, msg)
			setDeploymentCondition(&d.Status, *condition)
			needsUpdate = true
		}
		...
		if needsUpdate {
			var err error
			if d, err = r.kromeClient.AppsV1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d, metav1.UpdateOptions{}); err != nil {
				return nil, err
			}
		}
		return rsCopy, nil
	...
	*(newRS.Spec.Replicas) = newReplicasCount
	...
	case errors.IsAlreadyExists(err):
		...
		rs := &kromev1.ReplicaSet{}
		if err := r.client.Get(context.TODO(), types.NamespacedName{Name: createdRS.Name, Namespace: createdRS.Namespace}, rs); err != nil {
			return nil, err
		}
		...
		*d.Status.CollisionCount++
		...
		// error.
		_, dErr := r.kromeClient.AppsV1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d, metav1.UpdateOptions{})
		if dErr == nil {
	...
	case err != nil:
		...
		if hasProgressDeadline(d) {
			cond := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionFalse, deploymentutil.FailedRSCreateReason, msg)
			setDeploymentCondition(&d.Status, *cond)
			// We don't really care about this error at this point, since we have a bigger issue to report.
			// TODO: Identify which errors are permanent and switch DeploymentIsFailed to take into account
			// these reasons as well. Related issue: https://github.com/kubernetes/kubernetes/issues/18568
			_, _ = r.kromeClient.AppsV1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d, metav1.UpdateOptions{})
		}
	...
	needsUpdate := setDeploymentRevision(d, newRevision)
	if !alreadyExists && hasProgressDeadline(d) {
		msg := fmt.Sprintf("Created new replica set %q", createdRS.Name)
		condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionTrue, deploymentutil.NewReplicaSetReason, msg)
		setDeploymentCondition(&d.Status, *condition)
		needsUpdate = true
	}
	if needsUpdate {
		_, err = r.kromeClient.AppsV1().Deployments(d.Namespace).UpdateStatus(context.TODO(), d, metav1.UpdateOptions{})
	}
...
func (r *ReconcileDeployment) syncDeploymentStatus(allRSs []*kromev1.ReplicaSet, newRS *kromev1.ReplicaSet, d *kromev1.Deployment) error {
	...
	_, err := r.kromeClient.AppsV1().Deployments(newDeployment.Namespace).UpdateStatus(context.TODO(), newDeployment, metav1.UpdateOptions{})
	return err
}
...
func calculateStatus(allRSs []*kromev1.ReplicaSet, newRS *kromev1.ReplicaSet, deployment *kromev1.Deployment) kromev1.DeploymentStatus {
	...
	status := kromev1.DeploymentStatus{
		// TODO: Ensure that if we start retrying status updates, we won't pick up a new Generation value.
		ObservedGeneration:  deployment.Generation,
		Replicas:            getActualReplicaCountForReplicaSets(allRSs),
		UpdatedReplicas:     getActualReplicaCountForReplicaSets([]*kromev1.ReplicaSet{newRS}),
		ReadyReplicas:       getReadyReplicaCountForReplicaSets(allRSs),
	...
	if availableReplicas >= *(deployment.Spec.Replicas)-maxUnavailable(*deployment) {
		minAvailability := NewDeploymentCondition(kromev1.DeploymentAvailable, v1.ConditionTrue, deploymentutil.MinimumReplicasAvailable, "Deployment has minimum availability.")
		setDeploymentCondition(&status, *minAvailability)
	} else {
		noMinAvailability := NewDeploymentCondition(kromev1.DeploymentAvailable, v1.ConditionFalse, deploymentutil.MinimumReplicasUnavailable, "Deployment does not have minimum availability.")
		setDeploymentCondition(&status, *noMinAvailability)
	}
