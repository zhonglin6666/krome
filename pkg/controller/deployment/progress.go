...
func (r *ReconcileDeployment) syncRolloutStatus(allRSs []*kromev1.ReplicaSet, newRS *kromev1.ReplicaSet, d *kromev1.Deployment) error {
	...
	if hasProgressDeadline(d) && !isCompleteDeployment {
		...
		case deploymentProgressing(d, &newStatus):
			...
			condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionTrue, util.ReplicaSetUpdatedReason, msg)
		...
		case deploymentTimedOut(d, &newStatus):
			...
			condition := NewDeploymentCondition(kromev1.DeploymentProgressing, v1.ConditionFalse, util.TimedOutReason, msg)
			setDeploymentCondition(&newStatus, *condition)
		}
	}
	...
	if replicaFailureCond := r.getReplicaFailures(allRSs, newRS); len(replicaFailureCond) > 0 {
		...
		removeDeploymentCondition(&newStatus, kromev1.DeploymentReplicaFailure)
	}
...
func (r *ReconcileDeployment) getReplicaFailures(allRSs []*kromev1.ReplicaSet, newRS *kromev1.ReplicaSet) []kromev1.DeploymentCondition {
	var conditions []kromev1.DeploymentCondition
	if newRS != nil {
		for _, c := range newRS.Status.Conditions {
			if c.Type != kromev1.ReplicaSetReplicaFailure {
				continue
			}
			conditions = append(conditions, ReplicaSetToDeploymentCondition(c))
		}
	}
...
func (r *ReconcileDeployment) requeueStuckDeployment(d *kromev1.Deployment, newStatus kromev1.DeploymentStatus) time.Duration {
	currentCond := getDeploymentCondition(d.Status, kromev1.DeploymentProgressing)
	// Can't estimate progress if there is no deadline in the spec or progressing condition in the current status.
	if !hasProgressDeadline(d) || currentCond == nil {
		return time.Duration(-1)
	}
	// No need to estimate progress if the rollout is complete or already timed out.
	if deploymentComplete(d, &newStatus) || currentCond.Reason == util.TimedOutReason {
		return time.Duration(-1)
	}
	...
	//r.enqueueAfter(d, after+time.Second)
	return after
}
